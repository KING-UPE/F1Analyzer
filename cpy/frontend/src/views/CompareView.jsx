...
export default function CompareView() {
  ...
  const [raceConditionsA, setRaceConditionsA] = useState([{ id: Date.now(), field: 'track', operator: 'equals', value: 'Suzuka' }]);
  const [driverSetsA, setDriverSetsA] = useState([
    [{ id: Date.now(), field: 'driver_id', operator: 'equals', value: 'D001' }]
  ]);

  const [raceConditionsB, setRaceConditionsB] = useState([{ id: Date.now()+1, field: 'track', operator: 'equals', value: 'Suzuka' }]);
  const [driverSetsB, setDriverSetsB] = useState([
    [{ id: Date.now()+2, field: 'driver_id', operator: 'equals', value: 'D002' }]
  ]);
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      const [resA, resB] = await Promise.all([
        fetch('http://localhost:5000/api/search', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payloadA) }),
        fetch('http://localhost:5000/api/search', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payloadB) })
      ]);
