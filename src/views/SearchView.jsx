  ...
  return (
    <div>
      ...
      {selectedRace && (
        <RaceDetailsModal 
          race={selectedRace} 
          highlightDriverId={highlightDriver} 
                    ...
                    <table className="data-table">
                      ...
                      <tbody>
                        {results.data.slice(0, 50).map((r, i) => (
                          <tr 
                            key={i} 
                            onClick={() => { setSelectedRace(r); setHighlightDriver(null); }}
                            ...
                            <td>
                              <div style={{ display: 'flex', gap: '0.5rem', flexWrap:'wrap' }}>
                                {r.matched_drivers.map(d => (
                                  <span 
                                    key={d} 
                                    className="badge hover-pop" 
                                    style={{ fontSize:'0.65rem', cursor: 'pointer' }}
                                    onClick={(e) => { e.stopPropagation(); setSelectedRace(r); setHighlightDriver(d); }}
                                    title={`Click to view ${d} strategy in Race ${r.race_id}`}
                                  >
                                    {d}
                                  </span>
                                ))}
